...
SRA run files were downloaded using SRAtools and cleaned using Trimmomatic. [Reference Sequences](Reference_sequences.zip) and an example [code](q.down_trim_map_Carnivora.sh) for analysis are availble at the corresponding links.  Reads uniquely mapped to the reference transcripts were counted using Samtools to produce a [raw count file](RawCounts_Data.zip), and then normalized by size (kb) of the reference sequence. Runs that had low numbers of cleaned reads resulting in no mapping to either IGF1 or IGF2 were removed from the study, resulting in a [final file](MetaData_Counts_Cleaned.csv) containing 245 SRA rRuns representing 82 species.  
