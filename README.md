...
SRA run files were downloaded using SRAtools and cleaned using Trimmomatic to trim sequences for a Phred quality score of 20 and removing resulting reads fewer than 36 bp. [Reference Sequences](Reference_sequences.zip) and an example [code](q.down_trim_map_Carnivora.sh) for analysis are available at the corresponding links. For each major clade, an IGF1 and IGF2 reference transcript of the coding sequence (CDS) from a focal species was downloaded from ENSEMBLE, NCBI, or DRYAD (Table S1). All the species from a particular clade were mapped to the same focal reference sequences (see Table S1) using BWA MEM. Reads uniquely mapped to the reference transcripts were counted using Samtools idxstats to produce a [raw count file](RawCounts_Data.zip). The read counts were then normalized by size (kb) of the reference sequence. Runs that had low numbers of cleaned reads resulting in no mapping to either IGF1 or IGF2 were removed from the study, resulting in a [final file](MetaData_Counts_Cleaned.csv) containing 245 SRA runs representing 82 species.
...
There were violations of heteroscedasticity found within subsets of our data. Using the "bestNormalize" package, Box Cox and Ordered Quantile normalization were identified as the most appropriate means of normalization for our data set. The gene expression data was transformed using both methods, but neither addressed the issue of homoscedasticity within the data. When the transformed data was used in statistical analysis, while summary statistics changed slightly, there were no changes in statistical significance or broad conclusions drawn from the data. Given the linear models robust nature to violations of assumptions and its conservative nature with small samples sizes, we have chosen to keep the non-transformed data as is for ease of interpretation. 
